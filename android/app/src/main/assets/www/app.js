...
const AndroidBridge = window.Android || {
    showToast: (msg) => console.log("JS Bridge (Toast):", msg),
    scheduleNotification: (hour, minute) => console.log("JS Bridge (Notification):", hour, minute),
    setWallpaper: (base64) => console.log("JS Bridge (Wallpaper): Base64 data received"),
    updateWidget: (text, author) => console.log("JS Bridge (Widget):", text, author)
};
...
const translations = {
    sl: {
        ...
        notification_settings: "Nastavitve obvestil", notification_time: "Čas dnevnega obvestila",
        set_wallpaper_btn: "Nastavi kot ozadje", wallpaper_set_success: "Ozadje uspešno nastavljeno!"
    },
    en: {
        ...
        notification_settings: "Notification Settings", notification_time: "Daily notification time",
        set_wallpaper_btn: "Set as Wallpaper", wallpaper_set_success: "Wallpaper set successfully!"
    }
};
...
async function loadQuotes() {
    try {
        ...
        currentLang = lang;
    if (window.Android) {
        AndroidBridge.syncLanguage(currentLang);
    }
...
async function handleDateChange(dateString) {
    ...
    if (window.Android && currentQuote.text) {
        AndroidBridge.updateWidget(currentQuote.text, currentQuote.author || "");
    }
...
// === UTILITIES ===
function updateAndroidNotification(timeValue) {
    if (!timeValue) return;
    const [hour, minute] = timeValue.split(':').map(Number);
    localStorage.setItem('notificationTime', timeValue);
    AndroidBridge.scheduleNotification(hour, minute);
}

function showToast(msg) {
    // If running in Android, also send to native toast
    if (window.Android) {
